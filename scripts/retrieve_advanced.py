...
STOPWORDS = {
    "the", "and", "for", "with", "this", "that", "from", "into", "than", "then", "were", "been", "are", "was",
    "have", "has", "had", "using", "use", "used", "via", "our", "their", "your", "about", "between", "within",
    "without", "over", "under", "into", "onto", "through", "such", "also", "can", "may", "might", "not", "but",
    "how", "what", "when", "where", "why", "who", "which", "paper", "study", "results", "method", "methods",
}
...
def expand_query_text(base_query_text: str, domain: str, mode: str) -> str:
    expanded = base_query_text
    if mode in ("domain", "domain+prf") and domain:
        expanded = f"{expanded} {str(domain).strip()}"
    return expanded
...
def extract_prf_terms(
    ...
    query_tokens = set(_tokenize_terms(query_text))
    counter: Counter[str] = Counter()
    for text in candidate_texts:
        counter.update(_tokenize_terms(text))
    terms = [t for t, _ in counter.most_common(prf_add_terms * 4) if t not in query_tokens]
    return terms[:prf_add_terms]
...
def build_submission(
    ...
    for i, qid in iterator:
        ...
        q_text_expanded = expand_query_text(q_text, query_domains[qid], query_expansion_mode)
        bm25_scores = bm25.get_scores(q_text_expanded.lower().split())
        ...
        if query_expansion_mode in ("prf", "domain+prf"):
            prf_ids = top_ids[: min(prf_topk, len(top_ids))]
            prf_terms = extract_prf_terms(
                q_text_expanded,
                [corpus_id_to_text[cid] for cid in prf_ids],
                prf_add_terms=prf_add_terms,
            )
            if prf_terms:
                q_text_expanded = f"{q_text_expanded} {' '.join(prf_terms)}"
                bm25_scores = bm25.get_scores(q_text_expanded.lower().split())
                if use_nonlinear_fusion:
                    score = fuse_scores_nonlinear(
                        bm25_scores=bm25_scores,
        ...
        keyword_scores = boost_keyword_overlap(q_text_expanded, [corpus_id_to_text[cid] for cid in top_ids])
...
def main() -> None:
    ...
    parser.add_argument(
        "--query-expansion",
        type=str,
        default=QUERY_EXPANSION_MODE,
        choices=["none", "domain", "prf", "domain+prf"],
        help="Query expansion mode for BM25 scoring",
    )
    parser.add_argument("--prf-topk", type=int, default=PRF_TOPK, help="Top docs used for PRF term extraction")
    parser.add_argument("--prf-add-terms", type=int, default=PRF_ADD_TERMS, help="Number of PRF terms to append")
    parser.add_argument(
        "--no-progress",
        action="store_true",
