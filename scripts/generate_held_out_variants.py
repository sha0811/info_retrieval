"""
Generate multiple held_out submissions with different domain boost weights.
We'll submit each to codabench to find the optimal weight for held_out.

Also: try different cite weights.
"""
...
print(f"Held_out queries: {len(ft_q_ids)}")
print(f"BGE cite embeddings: {bge_cite_embs.shape}")
print(f"E5 cite embeddings: {e5_cite_embs.shape}")


def retrieve_held(w_domain, w_bge_cite, w_e5_cite, cite_top_k=1000, top_k=100):
    sub = {}
...
configs = [
    # Best train config
    ("best_train_wd030_wbc065_we5c035", 0.30, 0.65, 0.35),
    # Sweep domain weights
    ("wd015_wbc065_we5c035", 0.15, 0.65, 0.35),
    ("wd020_wbc065_we5c035", 0.20, 0.65, 0.35),
    ("wd025_wbc065_we5c035", 0.25, 0.65, 0.35),
    ("wd035_wbc065_we5c035", 0.35, 0.65, 0.35),
    ("wd040_wbc065_we5c035", 0.40, 0.65, 0.35),
    # Sweep cite weights at best domain
    ("wd030_wbc080_we5c040", 0.30, 0.80, 0.40),
    ("wd030_wbc100_we5c050", 0.30, 1.00, 0.50),
    ("wd030_wbc050_we5c025", 0.30, 0.50, 0.25),
    # No domain (to test if domain is hurting held_out)
    ("wd000_wbc065_we5c035", 0.00, 0.65, 0.35),
    # Higher cite + no domain
    ("wd000_wbc100_we5c050", 0.00, 1.00, 0.50),
    # Pure dual-dense (no domain, no cite)
    ("wd000_wbc000_we5c000_pure_dense", 0.00, 0.00, 0.00),
]

print("\nGenerating held_out submissions...")
for name, wd, wbc, we5c in configs:
    print(f"  {name} ...", end=" ", flush=True)
    sub = retrieve_held(wd, wbc, we5c)

    json_path = SUBMISSIONS_DIR / f"held_{name}.json"
    zip_path = SUBMISSIONS_DIR / f"held_{name}.zip"
...
print("\nDone. Submit these zips to codabench to find optimal held_out config.")
print("Compare against current best: 0.69")
